...
def check_collision(idx, box_local): # check if the particle has collided with a wall or another particle

    particles_list = box_local.particles_list # list of particles in the box
    ...
    for i in range(len(particles_list)): # loop over all particles
        ...
        elif np.linalg.norm(particle_1.pos-particles_list[i].pos) <= 2*r: # if the distance between particle_1 and another particle is less than 2r it means they collided
        ...
        elif (particle_1.pos[0]+r > box_local.ux or particle_1.pos[0]-r < box_local.lx) or (particle_1.pos[1]+r > box_local.uy or particle_1.pos[1]-r < box_local.ly):

            # if the particle has collided with a wall we reverse its velocity

            particle_1.vel = -particle_1.vel

def update(box_local, position_data_local):

    particles_list = box_local.particles_list # list of particles in the box

    for i in range(len(particles_list)): # loop over all particles

        check_collision(i, box_local) # check if the particle that corresponds to index i in the list of particles has collided
        ...
        position_data_local[i].append(particle.pos) # put the position in the data which we will plot later

for _ in range(iterations): # run the simulation

    update(box, position_data)
...
scatter_plot = ax.scatter(x, y, color = 'green') # [all particles][t = 0][x or y]
...
animation = FuncAnimation(fig, animate, interval = 0.0000000001) # function that makes the animation, interval is the microseconds between frames
